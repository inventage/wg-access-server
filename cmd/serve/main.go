...
func Register(app *kingpin.Application) *servecmd {
	...
	cli.Flag("disable-inactive", "Disable inactive device deletion").Envar("WG_DISABLE_INACTIVE").Default("false").BoolVar(&cmd.AppConfig.DisableInactive)
	cli.Flag("inactive-duration", "Duration after inactive device are deleted").Envar("WG_INACTIVE_DURATION").Default("4380h").DurationVar(&cmd.AppConfig.InactiveDuration)
...
func (cmd *servecmd) Run() {
	...
	// Services
	if err := deviceManager.StartSync(conf.DisableMetadata, conf.DisableInactive, conf.InactiveDuration); err != nil {
